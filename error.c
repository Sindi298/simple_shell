...
void _printerror(char *title, char *string, int element)
{
	...
	write(STDERR_FILENO, title, _strnlen(title));
	write(STDERR_FILENO, ": ", 2);
	write(STDERR_FILENO, head, _strnlen(head));
	write(STDERR_FILENO, ": ", 2);
	write(STDERR_FILENO, string, _strnlen(string));
	write(STDERR_FILENO, note, _strnlen(note));
